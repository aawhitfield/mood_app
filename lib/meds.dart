...
class MedsWidgetState extends State<MedsWidget> {
  String medicineName = 'Medicine Name';                                        // text that will be the name of the medicine to get stored into the calendar entry journal
  List<String> medicineList = <String>[];                                       // list of medicines to be saved for users to quickly reselect medicines
  ...
  void _selectMed() {                                                           // popup to select the medicine for the journal entry
    showDialog(
        context: context,
        child: Dialog(
          child: Column(
              ...
              Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    hintText: 'New medicine',                                   // prompts user to enter a new medicine
                    hintStyle: Theme.of(context).textTheme.headline,
                  ),
                  onSubmitted: (String text) {
                    setState(() {
                      medicineName = text;
                      medicineList.add(medicineName);                           // adds user entry to a list to save for quick re-selection
                      Navigator.pop(context);
                    });
                  },
                ),
              ),
              Expanded(                                                         // expanded prevents overflow on the ListView and makes it scrollable
                child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return new ListTile(
                        ...
                        onTap: (){
                          setState(() {
                            medicineName = medicineList[index];                   // if user selects a medicine on the list, sets label text
                            Navigator.pop(context);
                          });
                        },
                      );
  ...
  void addMedRecord() {                                                         // adds the med record to the general Journal, saves it to Shared Preferences for permanent storage
    setState(() {
      this.widget.parent.setState((){                                           // updates UI in this widget/class/file as well as in main.dart
      List<String> medAsAList = <String>[];                                     // place to store the med type as List to satisfy the requirements of the Entry Class
      medAsAList.add(medicineName);                                             // adds the medicine name to the medAsList to meet requirements of the Entry Class parameter type
      ...
      Entry newEntry = new Entry(combinedDateTime, eventNotes, medAsAList, EntryType.med);
                                                                                // creates a new Entry with all of the information the user has selected.

      this.widget.parent.journal.insert(0,newEntry);                            // adds the new Entry into the global journal to show up in Calendar View at the beginning of the list
      ...
      medAsAList.clear();                                                       // clears the list containing the medicine so it can be reused in the future
